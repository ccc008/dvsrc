...
//{$Id: ProcessKiller.pas 34 2011-03-10 04:48:59Z dvpublic0@gmail.com $}
...
begin
  ...
  if handle <> 0 then try
    ...
    if Process32First(handle, ProcStruct) then repeat
      s := PChar(@ProcStruct.szExeFile);
      if fMatchExeName(PWideChar(WideString(s))) then begin
...
begin
  ...
  len := lstrlenW(srcName);
  if (len >= len_sample) then begin
      pw := PWideChar(@srcName[len - len_sample]);
      if lstrcmpiW(pw, PWideChar(m_Sample)) = 0 then begin
        Result := true;
      end;
    end;
end;
...
begin
  Result := lstrcmpiW(srcName, PWideChar(m_Sample)) = 0;
end;
